    ...
    <aside className={open ? 'inspector inspector-open' : 'inspector'}>
      ...
      {step ? (
        <div className="inspector-form">
          <label>Key or mouse button<KeyCaptureButton value={step.key} label="Change" onChange={(key) => onStepChange({ ...step, key })} /></label>
          ...
          {timingIssue ? <div className="notice notice-error"><Activity size={16} /> {timingIssue.message}</div> : <div className="notice"><Activity size={16} /> The wait begins after this action is released.</div>}
        </div>
